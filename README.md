...
fun weekend project, and the outcome is a little lisp implementation
that supports
...
All those in  less than 1000 lines of C#. I didn't sacrifice readability for size.
The code is barely commented to help the reader understand
how all these features work.

Running LittleLisp
------------------

The project should compile without any external dependencies, other than the 
.NET framework version 4. It may compile under earlier versions, but I have not
tested.

To run the REPL interpreter simply type:

`LittleLisp`

This will drop you into a REPL that will allow you to type expressions and output
the return value.

To evaluate a file, simply pass it as a parameter on the command line like this:

`LittleLisp file.lisp`

This will execute the file and display the results of running it. Note that the value
of each expression will not be printed when executing a file.

Language features
-----------------

LittleLisp is a traditional Lisp interpreter. It reads one expression at a time
...
You can type expressions on multiple lines, as the interpreter counts the number
of open and close parentheses to determine when an expression is complete.

### Literals

LittleLisp supports integer literals, `()`, `t`, symbols, and list literals.
...
* Symbols are objects with unique name. They are used to represent identifiers.
  Because LittleLisp does not have string type, symbols are sometimes used as a
...
`()`. This is the only loop supported by LittleLisp.

If you are familiar with Scheme, you might be wondering if you could write a
loop by tail recursion in LittleLisp. The answer is no. Tail calls consume stack
space in LittleLisp, so a loop written as recursion will fail with the memory
exhaustion error.
...
LittleLisp supports variables and functions. They can be defined using `define`.
